...
class Monitor():
    ...
    def empty(self):
        return self.nc == 0 and self.np == 0
    ...
    def enter_car(self, cid : int, direction : int) -> None:
        ...
        self.mutex.wait_for(
            lambda : 
                (self.nc[index] < N_CARS_IN_BRIDGE and self.nc[1-index] == 0 and self.np.value == 0)
        )
        self.nc[index] += 1
        self.cars[index] -= 1
    ...
    def enter_pedestrian(self, pid : int) -> None:
        ...
        self.mutex.wait_for(
            lambda : 
                (self.np.value < N_PED_IN_BRIDGE and self.nc[0] == 0 and self.nc[1] == 0)
        )
